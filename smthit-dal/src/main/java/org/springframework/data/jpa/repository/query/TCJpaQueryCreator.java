...
import org.springframework.data.jpa.repository.support.TCJpaRepositoryImpl;
 ...
 */
public class TCJpaQueryCreator extends AbstractQueryCreator<CriteriaQuery<? extends Object>, Predicate> {
	 ...
	 * @param em
	 */
	public TCJpaQueryCreator(PartTree tree, ReturnedType type, CriteriaBuilder builder,
	...
	protected CriteriaQuery<? extends Object> complete(Predicate predicate, Sort sort,
		 ...
		 * 增加逻辑删除过滤的条件
		 */
		Predicate deletePredicate = null;
		if(EntityModel2.class.isAssignableFrom(returnedType.getDomainType())) {
			Path<Boolean> deletePath = root.<Boolean> get(TCJpaRepositoryImpl.DELETED_FIELD);
			deletePredicate = builder.isFalse(deletePath);
		}
		
		if(predicate != null) {
			if(null != deletePredicate) {
				predicate =  builder.and(predicate, deletePredicate);
			}
		} else {
			...
			switch (type) {
				...
				case NOT_CONTAINING:
					...
					if (property.getLeafProperty().isCollection()) {
						...
						return type.equals(NOT_CONTAINING) ? builder.isNotMember((Expression<?>)parameterExpression, propertyExpression)
								: builder.isMember((Expression<?>)parameterExpression, propertyExpression);
					}
