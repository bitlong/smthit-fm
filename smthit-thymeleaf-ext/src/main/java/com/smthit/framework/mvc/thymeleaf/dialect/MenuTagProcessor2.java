 ...
 * 菜单标签
 */
public class MenuTagProcessor2 extends AbstractElementTagProcessor {
	private static final String TAG_NAME = "menu";
	private static final int PRECEDENCE = 300;

	public MenuTagProcessor2(String dialectPrefix) {
	...
	private void createSubMenu(ITemplateContext context, IProcessableElementTag tag, IElementTagStructureHandler structureHandler, List<Menu> subMenus) {
		...
		attrs.put("class", tag.getAttributeValue("class"));
		attrs.put("lay-filter", tag.getAttributeValue("lay-filter"));
		...
		for(Menu menu : subMenus) {
			...
			IStandaloneElementTag aTag;
			if("folder".equalsIgnoreCase(menu.getType())) {
				aTag = modelFactory.createStandaloneElementTag("a", "href", "javascript:;", false, false);
				//<i class="layui-icon">&#xe625;</i>
			} else {
				IStandardExpression se = parser.parseExpression(context, "@{" + menu.getUrl() + "}");
				aTag = modelFactory.createStandaloneElementTag("a", "href", (String)se.execute(context), false, false);
			}
			...
			if(menu.getChildren() != null && menu.getChildren().size() > 0) {
				//dl
				IStandaloneElementTag dlTag = modelFactory.createStandaloneElementTag("ul", "class", "fastui-two-menu");
				model.add(dlTag);
				...
				for(Menu child : menu.getChildren()) {
					...
					//dd->a->i+cite
					IStandaloneElementTag ddTag = modelFactory.createStandaloneElementTag("li");
					model.add(ddTag);
					...
					attrs.put("href", (String)se.execute(context));
