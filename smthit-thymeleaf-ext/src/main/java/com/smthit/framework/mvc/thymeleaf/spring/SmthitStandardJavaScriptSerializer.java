...
import com.smthit.lang.json.jackson.SmthitHttpMessageObjectMapper;

public class SmthitStandardJavaScriptSerializer implements IStandardJavaScriptSerializer {
	...
	private String computeJacksonPackageNameIfPresent() {
		// We will try to know whether Jackson is present in a way that is as resilient
		// as possible with
	...
	public SmthitStandardJavaScriptSerializer(final boolean useJacksonIfAvailable) {
	...
	private static final class JacksonStandardJavaScriptSerializer implements IStandardJavaScriptSerializer {
		...
		JacksonStandardJavaScriptSerializer(final String jacksonPrefix) {
			...
			this.mapper = new SmthitHttpMessageObjectMapper();
			...
			this.mapper.setDateFormat(new JacksonThymeleafCommonDateFormat("yyyy-MM-dd HH:mm:ss"));

			/*
			 * Now try to (conditionally) initialize support for Jackson serialization of
			 * JSR310 (java.time) objects, by making use of the 'jackson-datatype-jsr310'
			 * optional dependency.
			 */
			final Class<?> javaTimeModuleClass = ClassLoaderUtils
					.findClass(jacksonPrefix + ".datatype.jsr310.JavaTimeModule");
	...
	/*
	 * This DateFormat implementation replaces the standard Jackson date
	 * serialization mechanism for ISO6801 dates, with the aim of making Jackson
	 * output dates in a way that is at the same time ECMAScript-valid and also as
	 * compatible with non-Jackson JavaScript serialization infrastructure in
	 * Thymeleaf as possible. For this:
	 *
	 * * The default Jackson behaviour of outputting all dates as GMT is disabled. *
	 * The default Jackson format adding timezone as '+0800' is modified, as
	 * ECMAScript requires '+08:00'
	 *
	 * On the latter point, see
	 * https://github.com/FasterXML/jackson-databind/issues/1020
	 */
	private static final class JacksonThymeleafISO8601DateFormat extends DateFormat {
		 ...
		 *
		 * The correct ISO8601 format would be "yyyy-MM-dd'T'HH:mm:ss.SSSXXX", but the
		 * "X" pattern (which outputs the timezone as "+02:00" or "Z" instead of
		 * "+0200") was not added until Java SE 7. So the use of this SimpleDateFormat
		 * object requires additional post-processing.
		 *
		 * SimpleDateFormat objects are NOT thread-safe, but it is here being used from
		 * another DateFormat implementation, so we must suppose that it is the use of
		 * this DateFormat wrapper that will be adequately synchronized by Jackson.
		 */
	...
	private static final class JacksonThymeleafCommonDateFormat extends DateFormat {
		...
		JacksonThymeleafCommonDateFormat(String pattern) {
			super();
		...
		@Override
		public Object clone() {
			JacksonThymeleafCommonDateFormat other = (JacksonThymeleafCommonDateFormat) super.clone();
			other.dateFormat = (SimpleDateFormat) dateFormat.clone();
			return other;
		}
	...
	/*
	 * This CharacterEscapes implementation makes sure that the slash ('/') and
	 * ampersand ('&') characters are also escaped, which is not standard Jackson
	 * behaviour.
	 *
	 * Escaping '/' covers against the possible premature closing of <script> tags
	 * inside inlined JavaScript literals, thus preventing code injection in
	 * templates being processed by browsers as HTML.
	 *
	 * Escaping '&' covers against the injection of XHTML-escaped code which might
	 * prematurely close the inlined JavaScript literals and even the container
	 * <script> tags in templates being processed by browsers as XHTML.
	 *
	 * Note that, unfortunately Jackson's escape customization mechanism offers no
	 * way to only escape '/' when it is preceded by '<', so that only '</' is
	 * escaped. Therefore, all '/' need to be escaped. Which is a difference with
	 * the default Unbescape-based mechanism.
	 */
	...
	private static final class DefaultStandardJavaScriptSerializer implements IStandardJavaScriptSerializer {
		...
		private static void writeString(final Writer writer, final String str) throws IOException {
			/*
			 * Note we will be using JSON escape instead of JavaScript escape. They are
			 * basically (99%) interchangeable once we have established that our literals
			 * use double-quotes (") and not single-quotes, and this allows us to avoid
			 * escaping single-quotes and therefore be more consistent with Jackson-based
			 * serialization, which is obviously JSON-based.
			 */
			writer.write('"');
			writer.write(JsonEscape.escapeJson(str, JsonEscapeType.SINGLE_ESCAPE_CHARS_DEFAULT_TO_UHEXA,
		...
		private static void writeArray(final Writer writer, final Object arrayObj) throws IOException {
			...
			if (arrayObj instanceof Object[]) {
				...
				throw new IllegalArgumentException("Cannot write value \"" + arrayObj + "\" of class "
						+ arrayObj.getClass().getName() + " as an array");
			}
			writer.write(']');
		}
		...
		private static void writeKeyValue(final Writer writer, final Object key, final Object value)
				throws IOException {
		...
		private static void writeObject(final Writer writer, final Object object) throws IOException {
			try {
				final PropertyDescriptor[] descriptors = Introspector.getBeanInfo(object.getClass())
				...
				throw new IllegalArgumentException(
						"Could not perform introspection on object of class " + object.getClass().getName(), e);
	...
	private void handleErrorLoggingOnJacksonInitialization(Throwable e) {
		final String warningMessage = "[THYMELEAF] Could not initialize Jackson-based serializer even if the Jackson library was "
		...
		if (logger.isDebugEnabled()) {
			...
			// We will avoid an ugly exception trace appearing through the logs as WARN
			// unless DEBUG is enabled
			logger.warn(warningMessage + " Set the log to DEBUG to see a complete exception trace. Exception "
